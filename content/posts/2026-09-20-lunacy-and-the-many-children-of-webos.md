...
- Lunacy is _not_ an emulator -- we have one of those, it’s not great, but it works.
...
- An app contract environment that hosts patched Enyo (and soon Mojo) framework libraries and a simulated service bus that allows existing webOS apps to run as-is.
- A webOS style keyboard that can be optionally installed to complete the experience
...
Lunacy brings a webOS compatibility environment to Android devices via an app, no unlocking required.
